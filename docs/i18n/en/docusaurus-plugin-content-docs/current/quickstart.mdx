...
`/etc/oxidns/config.yaml` and `/etc/oxidns/initial_password` are config files and are kept where possible during upgrades or package removal. Delete `/etc/oxidns` and `/var/lib/oxidns` manually when you want a full purge.
...
does not rewrite `/etc/oxidns/config.yaml` just to generate a password, so
routine upgrades do not repeatedly trigger `opkg` conffile-difference notices
because of password changes.
