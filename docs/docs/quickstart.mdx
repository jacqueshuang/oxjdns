...
* **OpenWrt / iStoreOS**：优先使用 GitHub Release 中的 `.ipk`
...
## 2. OpenWrt / iStoreOS 原生包

OxiDNS 的 GitHub Release 会提供 OpenWrt/iStoreOS 24.10 `.ipk` 包，第一版覆盖：

- `x86_64`
- `aarch64_cortex-a53`，面向 iStoreOS 常见 arm64 设备

下载同一架构下的两个包：

- `oxidns-openwrt-24.10.*-<架构>-oxidns_*.ipk`
- `oxidns-openwrt-24.10.*-<架构>-luci-app-oxidns_*.ipk`

安装：

```sh
opkg update
opkg install ./oxidns-openwrt-24.10.*-<架构>-oxidns_*.ipk \
  ./oxidns-openwrt-24.10.*-<架构>-luci-app-oxidns_*.ipk
```

安装后会自动执行：

- 生成随机 WebUI/API 密码，用户名为 `admin`
- 启用并启动 `/etc/init.d/oxidns`
- 使用 OpenWrt `procd` 托管服务

默认路径与端口：

| 项目 | 默认值 |
| --- | --- |
| 配置文件 | `/etc/oxidns/config.yaml` |
| 默认配置模板 | `/usr/share/oxidns/config.default.yaml` |
| 初始密码 | `/etc/oxidns/initial_password` |
| 工作目录 | `/var/lib/oxidns` |
| WebUI 静态文件 | `/usr/share/oxidns/webui` |
| DNS UDP/TCP | `:5335` |
...
也可以在 LuCI / iStoreOS 管理界面中打开 **服务 → OxiDNS**，查看服务状态、启动/停止/重启、查看或重置密码，并跳转到 OxiDNS 自带 WebUI。

如果安装后侧边栏暂时没有出现 OxiDNS，请先确认同时安装了 `luci-app-oxidns` 包，然后刷新 LuCI 菜单缓存：

```sh
opkg install ./oxidns-openwrt-24.10.*-<架构>-luci-app-oxidns_*.ipk
...
<Admonition type="warning" title="管理端口安全">

OpenWrt / iStoreOS 包默认允许局域网访问 `9199` 端口，并启用 Basic Auth。不要把该端口直接暴露到公网；如需远程管理，请使用 VPN、反向代理认证或防火墙白名单。
...
`/etc/oxidns/config.yaml` 和 `/etc/oxidns/initial_password` 是设备上的运行期配置，升级或卸载时会尽量保留。需要彻底清理时可手动删除 `/etc/oxidns` 和 `/var/lib/oxidns`。

随机 API 密码保存在 `/etc/oxidns/initial_password`，服务启动时会通过 `OXIDNS_API_PASSWORD` 传给 OxiDNS。包内只携带 `/usr/share/oxidns/config.default.yaml`，仅在缺少 `/etc/oxidns/config.yaml` 时才创建运行配置，因此正常升级不会反复触发 `opkg` 的 conffile 差异提示。

### 使用 OpenWrt SDK 自行构建

如果已经有交叉编译好的 `oxidns` 和 `webui` 静态文件，可以直接生成 Release 风格的 `.ipk`，不需要拉取或编译 OpenWrt SDK：
...
如果从源码构建，需要先准备 `webui/out`，或在执行 `make` 时传入 `OXIDNS_WEBUI_DIR`。如果使用已经构建好的二进制，可传入：
...
- 想在 OpenWrt / iStoreOS 上安装：使用 `.ipk` 服务包和 `luci-app-oxidns`
