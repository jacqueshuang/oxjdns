...
`/etc/oxidns/config.yaml` 和 `/etc/oxidns/initial_password` 是配置文件，升级或卸载时会尽量保留。需要彻底清理时可手动删除 `/etc/oxidns` 和 `/var/lib/oxidns`。

随机 API 密码保存在 `/etc/oxidns/initial_password`，服务启动时会通过 `OXIDNS_API_PASSWORD` 传给 OxiDNS。安装脚本不会为了生成密码而改写 `/etc/oxidns/config.yaml`，因此正常升级不会因为随机密码变化反复触发 `opkg` 的 conffile 差异提示。
