...
| OpenWrt / iStoreOS 24.10 x86_64 | `oxidns-openwrt-24.10.*-x86_64-*.ipk` |
| OpenWrt / iStoreOS 24.10 arm64 | `oxidns-openwrt-24.10.*-aarch64_cortex-a53-*.ipk` |
...
On Linux, prefer the `musl` build if you are unsure about compatibility. OpenWrt / iStoreOS users should use the `.ipk` packages; installation enables and starts the service automatically, with DNS on port `5335` and the WebUI/API on port `9199` by default.
